 ...
 * proportionally. Do not recolour the mark or set the wordmark in sans.
 */
...
function MarkShapes({ mono = false }: { mono?: boolean }) {
  const lineColor = mono ? 'currentColor' : PAPER;
  const chartColor = mono ? 'currentColor' : OCHRE;
  return (
    <>
      {!mono && <rect width="64" height="64" rx="14" fill={TEAL} />}
      {mono && (
        <rect
          x="1.5"
          y="1.5"
